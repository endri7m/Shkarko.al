...
exports.transcodeAudio = transcodeAudio;
...
const redisPublisher = new ioredis_1.default(redisUrl);
// Ensure scratch conversion folder exists
const SCRATCH_DIR = '/tmp/sonicflow-scratch';
const CONVERTED_DIR = path_1.default.join(SCRATCH_DIR, 'converted');
 ...
 */
async function publishJobProgress(jobId, status, progress, extra = {}) {
 ...
 */
function downloadThumbnail(url, jobId) {
    return new Promise((resolve) => {
        const outputPath = path_1.default.join(SCRATCH_DIR, `thumb_${jobId}.jpg`);
 ...
 */
async function transcodeAudio(job, options) {
    const { jobId, sourceType, sourcePath, sourceUrl, bitrate, sampleRate } = options;
    const outputPath = path_1.default.join(CONVERTED_DIR, `${jobId}.mp3`);
    ...
    if (sourceType === 'UPLOAD' && !fs_1.default.existsSync(inputSource)) {
        throw new Error(`Upload source file does not exist at path: ${inputSource}`);
    }
    return new Promise((resolve, reject) => {
        let command = (0, fluent_ffmpeg_1.default)(inputSource);
        ...
        command
            ...
            try {
                ...
                reject(new Error(`Failed to stat output file: ${e.message}`));
            }
        })
            .on('error', (err) => {
